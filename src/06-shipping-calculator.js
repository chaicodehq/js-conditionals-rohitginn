...
export function calculateShipping(weight, country, orderTotal) {
  ...
  if (country === 'US') {
    if (weight > 0 && weight <= 1) cost += 5;
    else if (weight > 1 && weight <= 5) cost += 10;
    else if (weight > 5) cost += 15;
  } 
  else {
    if (weight > 0 && weight <= 1) cost += 15;
    else if (weight > 1 && weight <= 5) cost += 25;
    else if (weight > 5) cost += 40;
  }
